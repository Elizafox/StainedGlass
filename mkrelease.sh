...
if [ -z "$MCP" -o -z "$VERSION" ]; then
	...
	echo "    produces stainedglass-0.1dev-client-1.2.5.jar and stainedglass-0.1dev-server-1.2.5.jar in release/"
	echo ""
	echo "The following files are copied into the jar with the following mappings:"
	echo "  art/stainedglass.png => elizacat/mc/StainedGlass/block.png"
...
JAR_CLIENT="$DIR_RELEASE/stainedglass-${VERSION}-client-${VERSION_CLIENT}.jar"
JAR_SERVER="$DIR_RELEASE/stainedglass-${VERSION}-server-${VERSION_SERVER}.jar"
